 ...
 * @version 1.0
 */
public class RandomDropSession extends Session implements InitializableSession {
     ...
     * @param l
     */
    public RandomDropSession(RandomDropLayer l) {
        super(l);
    }
    ...
    public void handle(Event ev) {
        ...
        try {
            if(ev instanceof MyEccoEvent) {
                    ...
                    if(ev.getDir() == Direction.DOWN && ev instanceof MyEccoEvent) {
            ...
            if (echo.getDir() == Direction.DOWN) { //PASSING EVERYTHING DOWN!!!!!!
                ...
                if(echo.getDir() == Direction.UP){
                    if (echo.getChannel().isStarted()) {
                        try {
                            echo.go();
                            System.out.println("[mwseq_num="+(msgcount)+"]");
